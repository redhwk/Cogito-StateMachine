...
func _state_input(_event: InputEvent) -> void:
	...
	if input_settings:
		if input_settings.toggle_crouch and Input.is_action_just_pressed(InputConstants.INPUT_CROUCH):
			try_crouch = !try_crouch
		elif not input_settings.toggle_crouch:
...
func _update(delta: float) -> void:
	...
	if not try_crouch and not (crouch_raycast and crouch_raycast.is_colliding()):
		finished.emit("Stand")
		return
